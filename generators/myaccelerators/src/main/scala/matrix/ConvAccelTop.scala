...
when(state === sIdle && io.start) {
  ...
  // Pipeline-delay storeValid to match 6-cycle ConvUnit latency:
  //   colValid -> RegNext(1) -> ShiftRegister(5) -> outValid  (6 cycles total)
  //   storeValidD6 is true exactly when outValid fires for a real image column
  val storeValidD1 = RegNext(lineBuf.io.storeValid, false.B)
  val storeValidD2 = RegNext(storeValidD1, false.B)
  val storeValidD3 = RegNext(storeValidD2, false.B)
  val storeValidD4 = RegNext(storeValidD3, false.B)
  val storeValidD5 = RegNext(storeValidD4, false.B)
  val storeValidD6 = RegNext(storeValidD5, false.B)
...
//  printf("engineStall=%d\n", engine.io.stall)
 ...
 when(state === sIdle && io.start) {
  ...
  // Use pipeline-delayed storeValid instead of storeColCnt for correct column alignment
  storeQueue.io.enq.valid := engine.io.outValid && storeValidD6
